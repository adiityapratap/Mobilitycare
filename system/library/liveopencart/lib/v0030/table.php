...
namespace liveopencart\lib\v0030;

class Table extends Library {
	
	// [[name=>'abc', type=>' int(11) NOT NULL AUTO_INCREMENT  ', key|primary=>true, cast=>'float|int|string'], [...], ...]
	protected $name          = '';
	...
	public function __construct($registry, $create_if_not_exists = false) {
		call_user_func_array( parent::class.'::__construct' , func_get_args());
		
		$this->init($create_if_not_exists);
	}
	...
	protected function init($create_if_not_exists = false) {
	...
	protected function checkColumns() {
		...
		if ( $this->exists() ) {
			foreach ( $this->columns as $column ) {
				...
				$this->simple_db->addTableColumnIfNotExists($this->name, $column['name'], $type);
				if ( !empty($column['key']) ) {
	...
	//			strings
	//			int(ids)
	//			arrays [ [column_name, condition, value/values], .. ]
	...
	public function updateById($id, $data, $sql_sets = []) {
		$this->update((int)$id, $data, $sql_sets);
		return $id;
	}
