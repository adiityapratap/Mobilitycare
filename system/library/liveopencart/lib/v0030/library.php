...
class library {
	...
	protected static function initInstance($registry, $args = []) {
	...
	protected function getLibraryInstanceByName($lib_name) {
		...
		return forward_static_call_array([$lib_name, 'getInstance'], $args);
	...
	protected function getOuterLibraryInstanceByName($lib_name) {
		...
		if (method_exists($instance, 'init')) {
			call_user_func_array( [$instance, 'init'] , array_slice(func_get_args(), 1));
		}
		return $instance;
	}
