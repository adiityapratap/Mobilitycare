...
class ro extends \liveopencart\lib\v0030\extension {
	
	protected $extension_code = 'ro3';
	protected $version        = '3.2.0';

	protected $events_admin = [
		'view/common/footer/after' => 'extension/module/related_options/eventViewCommonFooterTemplateAfter',
	];
	...
	public function getSettings() {
		if ( !$this->hasCacheSimple(__FUNCTION__) ) {
			...
			$this->setCacheSimple(__FUNCTION__, $ro_settings ? $ro_settings : [] );
		}
	...
	public function callPRO($method_name, $args = []) {
		if ( $this->versionPRO() ) {
			return call_user_func_array([\liveopencart\ext\ropro::getInstance($this->registry), $method_name], $args);
		}
	}
	
	public function callPOIU($method_name, $args = []) {
		if ( class_exists('\liveopencart\ext\poiu') ) {
			return call_user_func_array([\liveopencart\ext\poiu::getInstance($this->registry), $method_name], $args);
		}
	}
	
	public function callPOIP($method_name, $args = []) {
		if ( class_exists('\liveopencart\ext\poip') ) {
			return call_user_func_array([\liveopencart\ext\poip::getInstance($this->registry), $method_name], $args);
		}
	}
	...
	public function updateProductAndProductOptionQuantitieyByROIds($ro_ids) {
		if (empty($ro_ids)) {
			return;
		}
		$ro_ids = array_map('intval', $ro_ids);
		$query = $this->db->query("SELECT DISTINCT product_id FROM `".DB_PREFIX."relatedoptions` WHERE `relatedoptions_id` IN (".implode(',', $ro_ids).") ");
		foreach ( $query->rows as $row ) {
			$this->updateProductAndProductOptionQuantitiesByRO($row['product_id']);
		}
	}

	public function updateProductAndProductOptionQuantitiesByRO($product_id) {
		...
		foreach ( $ro_data as $ro_dt ) {
			...
			foreach ( $ro_dt['ro'] as $ro_comb ) {
				...
				if ( $this->getStatusSeparatePOVForCustomerGroups() ) {
			...
			if (count($options)) {
				...
				foreach ($options as $option_id => $option_values) {
					...
					foreach ($option_values as $option_value_id => $option_cgs_datas) {
						...
						$product_option_value_id = $this->getProductOptionValueIdByOptionValueId($product_id, $product_option_id, $option_value_id);

						$option_datas = [['quantity' => array_sum(array_column($option_cgs_datas, 'quantity'))]];

						foreach ( $option_datas as $cg_key => $option_data ) {
							// simplified, without the option/qty per customer group support
	...
	protected function getProductOptionIdByOptionId($product_id, $option_id) {
		...
		return $query->row['product_option_id'] ?? null;
	}

	protected function getProductOptionValueIdByOptionValueId($product_id, $product_option_id, $option_value_id) {
		$query = $this->db->query("SELECT product_option_value_id FROM " . DB_PREFIX . "product_option_value WHERE product_option_id = " . (int)$product_option_id . " AND option_value_id = " . (int)$option_value_id . " ");
	...
	public function getROCombsByPOIdsAssocParams($params = []) {
		$defaults = [
			'use_cache'                 => false,
			...
			'allow_empty_options_array' => false,
		];
		...
		return $this->getROCombsByPOIds($params['product_id'], $params['options'], $params['use_cache'], $params['allow_zero_quantity'], $params['use_ro_data_cache'], $params['strict'], $params['for_front_end'], $params['include_fields'], $params['allow_empty_options_array']);
	}
	...
	public function getROCombsByPOIds($product_id, $param_options, $use_cache = false, $p_allow_zero_quantity = -1, $use_ro_data_cache = false, $strict = true, $for_front_end = false, $include_fields = false, $allow_empty_options_array = false) {
		
		if (!is_array($param_options) || (count($param_options) == 0 && !$allow_empty_options_array) ) {
			return FALSE;
		}
		...
		$cache_key = json_encode( [$customer_group_id, $product_id, $options, $p_allow_zero_quantity, $strict, $for_front_end, $include_fields] );
		...
		$matches = [];
		$ro_data = $this->getROData($product_id, $for_front_end, $p_allow_zero_quantity, $use_ro_data_cache, $include_fields);
		
		foreach ($ro_data as $ro_dt) {
			
			$options_values_to_check = [];
			...
			foreach ($ro_dt['ro'] as $ro_comb) {
				...
				if ( $strict && $options_values_to_check == $ro_comb['options'] ) {
					...
					$matches[] = $ro_comb;
				} elseif ($allow_empty_options_array && !$strict && !$options_values_to_check) {
					$matches[] = $ro_comb;
				}
			}
		}
	...
	public function getROData($product_id, $for_front_end = false, $p_allow_zero_quantity = -1, $use_cache = false, $include_fields = false, $include_quantity = false, $after_method = false) {
		...
		$cache_key = json_encode( [$product_id, $for_front_end, $p_allow_zero_quantity, $customer_group_id, $include_fields, $include_quantity,  $lang_id] );
		if ( $use_cache && $this->hasCache(__FUNCTION__, $cache_key) ) {
			return $this->getCache(__FUNCTION__, $cache_key);
		} else {
			...
			if ( $this->hasProductRO($product_id) ) {
				...
				foreach ($rovp_rows as $rovp_row) {
					...
					$ro_data[] = [	'rovp_id' => $rovp_row['relatedoptions_variant_product_id'],
					...
					foreach ($query->rows as $row) {
						...
						foreach (['length', 'width', 'height'] as $size_key) {
							$row[$size_key] = (float)( (!empty($row[$size_key]) && (float)$row[$size_key]) ? $row[$size_key] : $product[$size_key]);
							if ($for_front_end) {
								$row[$size_key.'_f'] = $this->length->format($row[$size_key], $product['length_class_id']);
							}
						}
						
						//if ($for_front_end) {
						//	$row['length_f'] = $this->length->format( ((!empty($row['length']) && (float)$row['length']) ? $row['length'] : $product['length']), $product['length_class_id']);
						//	$row['width_f'] = $this->length->format( ((!empty($row['width']) && (float)$row['width']) ? $row['width'] : $product['width']), $product['length_class_id']);
						//	$row['height_f'] = $this->length->format( ((!empty($row['height']) && (float)$row['height']) ? $row['height'] : $product['height']), $product['length_class_id']);
						//}
					...
					$query = $this->db->query("
						SELECT ROO.*, POV.product_option_id, POV.product_option_value_id, OV.sort_order
						FROM 	(	SELECT ROO.*
					...
					foreach ($query->rows as $row) {
						if ( isset($ro_data[$cnt]['ro'][$row['relatedoptions_id']]) ) {
							...
							$ro_data[$cnt]['ro'][$row['relatedoptions_id']]['ov_sort_order'.$row['product_option_id']] = $row['sort_order'];
							if (!$for_front_end) {
					...
					if ( $for_front_end ) {
						if ( !$ro_data[$cnt]['ro'] ) {
							$ro_data[$cnt]['ro'][]        = ['relatedoptions_id' => 0, 'options' => []];
	...
	private function getArrayColumnWithEmpty($arr, $col) {
		return array_map(function($el) use ($col) {
			return $el[$col] ?? null;
		}, $arr);
	}
	
	public function orderRODataByOVSortOrder($ro_data) {
		$ro_settings = $this->config->get('related_options');
		...
		if (!empty($ro_settings['spec_sort_order'])) {
			...
			foreach ($ro_data as &$ro_dt) {
				if (count($ro_dt['options_ids']) !== 0) {

					$ro_dt['ro']    = array_values($ro_dt['ro']);
					$sort_arguments = [];
					foreach ($ro_dt['options_ids'] as $option_id) {
						$sort_arguments[] = $this->getArrayColumnWithEmpty($ro_dt['ro'], 'ov_sort_order'.$option_id);
					}
	...
	public function updateOrderProductAdditionalFields($product, $order_product_id) {
		...
		if ($this->installed()) {
			...
			foreach (['length', 'width', 'height'] as $size_key) {
				if (!empty($ro_settings['spec_'.$size_key])) {
					$this->db->query("UPDATE " . DB_PREFIX . "order_product SET `".$size_key."`='".(float)($custom_fields[$size_key] * $quantity)."' WHERE order_product_id = " . (int)$order_product_id . "");
				}
			}
	...
	public function getProductInfoForROId($product, $ro_id, $ro_data = false, $ro_comb = false) {
		...
		if ( $product && $ro_id ) {
			...
			if ( $ro_comb ) {
				...
				$ro_custom_fields = $this->getCustomFields($product, [$ro_comb]);
				...
				if ( $this->installedLivePrice() ) { // allows to take standard product option prices into account
					...
					if ( class_exists('\\liveopencart\\ext\\liveprice') ) { // new LP
						$lp_prices = \liveopencart\ext\liveprice::getInstance($this->registry)->getCalc()->getProductPriceByParamsArray( [
						...
						$lp_data = $this->model_extension_liveopencart_liveprice->getProductPriceByParamsArray( [
							'product_id'        => $product['product_id'],
							'quantity'          => 1,
							'options'           => $ro_comb['options'],
							'use_ro_data_cache' => true, // always use caches
					...
					$ro_price_data = $this->calcProductPriceWithRO($product['price'], [$ro_comb], $product['special']);
	...
	public function checkTableOrderProduct() {
		...
		if (!empty($ro_settings['spec_length'])) {
			$this->simple_db->addTableColumnIfNotExists('order_product', 'length', 'decimal(15,8)');
		}
		
		if (!empty($ro_settings['spec_width'])) {
			$this->simple_db->addTableColumnIfNotExists('order_product', 'width', 'decimal(15,8)');
		}
		
		if (!empty($ro_settings['spec_height'])) {
			$this->simple_db->addTableColumnIfNotExists('order_product', 'height', 'decimal(15,8)');
		}
	...
	public function getCustomFields($product_info, $ro_combs) {
		...
		foreach (['length', 'width', 'height'] as $size_key) {
			$ro_adds[$size_key] = $product_info[$size_key] ?? 0;
		}
		...
		if ($ro_combs) {
			...
			foreach ($ro_combs as $ro_comb) {
				...
				foreach (['length', 'width', 'height'] as $size_key) {
					if (!empty($ro_settings['spec_'.$size_key])) {
						$ro_data[$size_key] = (float)(!empty($ro_comb[$size_key]) ? $ro_comb[$size_key] : $product_info[$size_key]);
					}
				}
			}
		}
		
		$ro_data['codes']  = $ro_adds;
		$ro_data['weight'] = $ro_weight;
			...
			if ( $ro_settings && is_array($ro_settings) && count($options) ) {
				...
				if ($ro_combs) {
					...
					if ( !empty($ro_settings['spec_weight']) ) {
						...
						foreach (['length', 'width', 'height'] as $size_key) {
							if (!empty($ro_custom_fields[$size_key])) {
								$data[$size_key] = $ro_custom_fields[$size_key];
							}
						}
					}
					return $data;
	...
	public function calcProductPriceWithRO($product_price, $ro_combs, $special = 0, $stock = false, $ro_price_modificator = 0, $quantity = false) {
		...
		if ($this->installed()) {
			...
			if (!$special_is_overriden && $special) {
				$special += $ro_price_modificator;
			}
	...
	public function getCompatibleOptionValues() {
		...
		foreach ($compatible_options as $option_id => $option_name) {
			$optsv[$option_id] = ['name' => $option_name, 'values' => []];
			$str_opt .= ",".$option_id;
		}
		if ($str_opt != "") {
		...
		if ( !$this->hasCacheSimple(__FUNCTION__) ) {
			...
			foreach ( $query->rows as $row ) {
				if ( isset($options[$row['option_id']]) ) {
					$options[$row['option_id']]['values'][$row['option_value_id']] = [
	...
	public function getProductMinROCombQuantityByOptions($product_id, $options = []) {
		$ro_combs = $this->getROCombsByPOIds($product_id, $options);
		$quantity = false;
		if ( $ro_combs ) {
			foreach ( $ro_combs as $ro_comb ) {
