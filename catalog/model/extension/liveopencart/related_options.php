...
class ModelExtensionLiveopencartRelatedOptions extends Model {
	...
	public function getBasicScripts() {
		...
		$basic_theme_script = $this->getBasicScriptTheme();
		if ( $basic_theme_script ) {
			$scripts[] = $basic_theme_script;
		}
	...
	public function getBasicScriptTheme() {
		$script_path = 'view/theme/extension_liveopencart/related_options/themes/'.$this->getThemeName().'/basic.js';
		if ( file_exists(DIR_APPLICATION.$script_path) ) {
			return $this->getScriptPathWithVersion($script_path);
		}
		$script_path = 'view/theme/extension_liveopencart/related_options/themes/'.strtolower($this->getThemeName()).'/basic.js';
		if ( file_exists(DIR_APPLICATION.$script_path) ) {
			return $this->getScriptPathWithVersion($script_path);
		}
	}
	...
	public function getDefaultRelatedOptionsForCart($product_id) {
		$ro_settings = $this->config->get('related_options');
		
		if ($this->installed() && !empty($ro_settings['defaults_to_cart'])) {
			$options = $this->getDefaultRelatedOptions($product_id);

			if (class_exists('\liveopencart\ext\pcop') && \liveopencart\ext\pcop::getInstance($this->registry)->installed() ) {

				$po_ids_to_display = $this->liveopencart_ext_pcop->getDisplayedProductOptionsIdsBySelectedProductOptionSet($product_id, $options);
				$options = array_filter($options, function($po_id) use ($po_ids_to_display) {
					return in_array($po_id, $po_ids_to_display);
