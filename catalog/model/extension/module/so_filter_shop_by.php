...
class ModelExtensionModuleSofiltershopby extends Model {
	...
	public function getProducts($opt_value_id,$att_value_id,$manu_value_id,$text_search,$minPrice,$maxPrice,$subcate_value_id,$category_id,$condition_search, $sort_by, $order_by, $p_start, $p_limit){
		...
		if($text_search != "")
		{
			$sql .= "\n(LCASE(pd.name) LIKE '%".utf8_strtolower($this->db->escape($text_search))."%' OR LCASE(p.model) = '" . $this->db->escape(utf8_strtolower($text_search)) . "' OR LCASE(p.sku) = '" . $this->db->escape(utf8_strtolower($text_search)) . "' OR LCASE(p.upc) = '" . $this->db->escape(utf8_strtolower($text_search)) . "' OR LCASE(p.ean) = '" . $this->db->escape(utf8_strtolower($text_search)) . "' OR LCASE(p.jan) = '" . $this->db->escape(utf8_strtolower($text_search)) . "' OR LCASE(p.isbn) = '" . $this->db->escape(utf8_strtolower($text_search)) . "' OR LCASE(p.mpn) = '" . $this->db->escape(utf8_strtolower($text_search)) . "') AND";
		}
		...
		// $sql .= "\nORDER BY $sort_by $order_by LIMIT $p_limit";
		// echo $sql;
		$query = $this->db->query($sql);
		$product_data = array();
	...
	public function getTotalProducts($data = array()) {
		...
		if($data['text_search'] != "")
		{
			$sql .= "\n(LCASE(pd.name) LIKE '%".utf8_strtolower($this->db->escape($data['text_search']))."%' OR LCASE(p.model) = '" . $this->db->escape(utf8_strtolower($data['text_search'])) . "' OR LCASE(p.sku) = '" . $this->db->escape(utf8_strtolower($data['text_search'])) . "' OR LCASE(p.upc) = '" . $this->db->escape(utf8_strtolower($data['text_search'])) . "' OR LCASE(p.ean) = '" . $this->db->escape(utf8_strtolower($data['text_search'])) . "' OR LCASE(p.jan) = '" . $this->db->escape(utf8_strtolower($data['text_search'])) . "' OR LCASE(p.isbn) = '" . $this->db->escape(utf8_strtolower($data['text_search'])) . "' OR LCASE(p.mpn) = '" . $this->db->escape(utf8_strtolower($data['text_search'])) . "') AND";
		}
