<?php
class ModelExtensionModuleSolistingtabs extends Model {
	...
	public function getProducts_listing_tabs($data = array()) {
		...
		$sql = "SELECT p.product_id,p.featured_image";
	...
	function getProductSpecial($product_id) {
		$sql = "SELECT price AS special FROM " . DB_PREFIX ."product_special ps WHERE ps.product_id = ". $product_id ." AND ps.customer_group_id = '". (int)$this->config->get('config_customer_group_id') ."' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1";
		
		$query = $this->db->query($sql);

		return isset($query->row['special']) ? $query->row['special'] : false;
	}
}