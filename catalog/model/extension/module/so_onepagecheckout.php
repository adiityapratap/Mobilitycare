...
class ModelExtensionModuleSoOnepageCheckout extends Model {
    ...
    'ndis_additional_notes'     => '',
    'ndis_consent_accuracy'     =>  '',
    'ndis_consent_processing'   =>  '',
    'affiliate_id'              => '',
    'commission'                => '',
    'marketing_id'              => '',
    'tracking'                  => ''
        ...
        $this->db->query("UPDATE `" . DB_PREFIX . "order` 
            SET invoice_prefix = '" . $this->db->escape($data['invoice_prefix']) . "',
                ...
                accept_language = '" . $this->db->escape($data['accept_language']) . "',
                 ndis_full_name = " . (isset($data['ndis_full_name']) ? "'" . $this->db->escape($data['ndis_full_name']) . "'" : "NULL") . ",
ndis_participant_number = " . (isset($data['ndis_participant_number']) ? "'" . $this->db->escape($data['ndis_participant_number']) . "'" : "NULL") . ",
ndis_dob = " . (isset($data['ndis_dob']) ? "'" . $this->db->escape($data['ndis_dob']) . "'" : "NULL") . ",
ndis_email = " . (isset($data['ndis_email']) ? "'" . $this->db->escape($data['ndis_email']) . "'" : "NULL") . ",
ndis_phone = " . (isset($data['ndis_phone']) ? "'" . $this->db->escape($data['ndis_phone']) . "'" : "NULL") . ",
ndis_requestor = " . (isset($data['ndis_requestor']) ? "'" . $this->db->escape($data['ndis_requestor']) . "'" : "NULL") . ",
ndis_plan_type = " . (isset($data['ndis_plan_type']) ? "'" . $this->db->escape($data['ndis_plan_type']) . "'" : "NULL") . ",
ndis_plan_start_date = " . (isset($data['ndis_plan_start_date']) ? "'" . $this->db->escape($data['ndis_plan_start_date']) . "'" : "NULL") . ",
ndis_plan_end_date = " . (isset($data['ndis_plan_end_date']) ? "'" . $this->db->escape($data['ndis_plan_end_date']) . "'" : "NULL") . ",
ndis_plan_manager_name = " . (isset($data['ndis_plan_manager_name']) ? "'" . $this->db->escape($data['ndis_plan_manager_name']) . "'" : "NULL") . ",
ndis_plan_manager_email = " . (isset($data['ndis_plan_manager_email']) ? "'" . $this->db->escape($data['ndis_plan_manager_email']) . "'" : "NULL") . ",
ndis_plan_manager_phone = " . (isset($data['ndis_plan_manager_phone']) ? "'" . $this->db->escape($data['ndis_plan_manager_phone']) . "'" : "NULL") . ",
ndis_additional_notes = " . (isset($data['ndis_additional_notes']) ? "'" . $this->db->escape($data['ndis_additional_notes']) . "'" : "NULL") . ",
ndis_consent_accuracy = " . (isset($data['ndis_consent_accuracy']) ? (int)$data['ndis_consent_accuracy'] : "NULL") . ",
ndis_consent_processing = " . (isset($data['ndis_consent_processing']) ? (int)$data['ndis_consent_processing'] : "NULL") . ",
                date_modified = NOW() 
    ...
    // 🔹 Recalculate shipping if postcode is set
    if (isset($this->session->data['payment_postcode'])) {
        ...
        // 🔹 Overwrite existing shipping total if found
        foreach ($totals as &$row) {
            if ($row['code'] === 'shipping') {
                // adjust total value (remove old, add new)
                $total -= $row['value'];
                $row['value'] = $shippingCost;
                $total += $shippingCost;
                break;
            }
        }
    ...
    // sort totals again in case values changed
    $sort_order = array();
    foreach ($totals as $key => $value) {
        $sort_order[$key] = $value['sort_order'];
    }
    ...
    public function getProducts() {
        ...
        if ($this->config->get('module_so_onepagecheckout_status') && (int)$this->config->get('module_so_onepagecheckout_product_image_width') > 0) {
            $width = (int)$this->config->get('module_so_onepagecheckout_product_image_width');
        }
        if ($this->config->get('module_so_onepagecheckout_status') && (int)$this->config->get('module_so_onepagecheckout_product_image_height') > 0) {
            $height = (int)$this->config->get('module_so_onepagecheckout_product_image_height');
        }
    ...
    public function getPaymentMethodCode() {
        ...
        if (isset($this->session->data['payment_method'])) {
            ...
            $this->session->data['payment_method'] = $this->session->data['payment_method'];
        }
        elseif ($this->config->get('module_so_onepagecheckout_default_payment')) {
            $code = $this->config->get('module_so_onepagecheckout_default_payment');
            if (isset($this->session->data['payment_methods'][$code])) {
                $this->session->data['payment_method'] = $this->session->data['payment_methods'][$code];
            }
        }
        else {
            $code = array_get_first_key($this->session->data['payment_methods']);
            if ($code !== false) {
                $this->session->data['payment_method'] = $this->session->data['payment_methods'][$code];
            }
        }
    ...
    public function getShippingMethodCode() {
        ...
        elseif ($this->config->get('module_so_onepagecheckout_default_shipping')) {
            $code = $this->config->get('module_so_onepagecheckout_default_shipping');
            $this->session->data['shipping_method'] = $this->session->data['shipping_methods'][$code]['quote'][$code];
        }
        else {
            $part1 = array_get_first_key($this->session->data['shipping_methods']);
            $part2 = array_get_first_key($this->session->data['shipping_methods'][$part1]['quote']);
            if (isset($this->session->data['shipping_methods'][$part1]['quote'][$part2])) {
                $code = $part1;
    ...
    public function setAddress($type, $address) {
        if (!empty($address)>0 && is_array($address)) {
            foreach ($address as $key => $value) {
                $this->order_data[$type . '_' . $key] = $value;
            }
        }
        if (!empty($this->getAddress($type))>0 && is_array($this->getAddress($type))) {
