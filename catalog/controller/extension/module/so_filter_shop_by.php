...
class ControllerExtensionModuleSofiltershopby extends Controller {
	...
	public function filter_data(){
		...
		$condition_search = $_POST['condition_search'];
		...
		$product_data = $this->model_extension_module_so_filter_shop_by->getProducts($opt_value_id,$att_value_id,$manu_value_id,$text_search,$minPrice,$maxPrice,$subcate_value_id,$category_id,$condition_search, $sort_by, $order_by, $start, $limit);
		if($product_data != "" && count($product_data) > 0){
			...
			foreach ($product_data as $result) {
				...
				$data['products'][] = array(
					...
					'href_quickview'        => str_replace('&amp;', '&', $this->url->link('extension/soconfig/quickview&product_id='.$result['product_id'] )),
		...
		$product_total = $this->model_extension_module_so_filter_shop_by->getTotalProducts($filter_data);
		$url = '';
