...
class ControllerExtensionModuleSolistingtabs extends Controller {
    ...
    //     exit;
		...
		$this->document->addStyle('catalog/view/javascript/so_listing_tabs/css/so-listing-tabs.css?v3');
		...
		if($is_ajax && isset($_POST['is_ajax_listing_tabs']) && $_POST['is_ajax_listing_tabs']){
			...
			$display_explore_benefits 		= (int)$setting['display_explore_benefits'] ;
			...
			$data_template = array(
				...
				'display_explore_benefits' 	=> $display_explore_benefits,
			...
			// for shpwing product for "get quote request" modal 
		...
		if ($cat['count'] > 0) 
		{
			...
			foreach($products_arr as $product_info)
			{
				...
				if ($product_info['image'] && $setting['product_get_image_data']) {
					...
					$featureImage = $this->model_tool_image->resize($product_info['featured_image'], $setting['width'], $setting['height']);
				...
				$description = (($setting['description_maxlength'] != 0 && strlen($product_info['short_description']) > $setting['description_maxlength']) ? utf8_substr(strip_tags(html_entity_decode($product_info['short_description'], ENT_QUOTES, 'UTF-8')), 0, $setting['description_maxlength']) . '..' : $product_info['short_description']);
				...
				$cat['child'][] = array(
					...
					'display_add_to_cart'  => $product_info['display_add_to_cart'],
					'display_custom_quote'  => $product_info['display_custom_quote'],
					'thumb'       	=> $image,
