...
class ControllerExtensionModuleSoinstagramgallery extends Controller {
	public function index($setting) {
		...
		if ($use_cache){
			...
			if (!$_data) {
				$url = 'https://api.instagram.com/v1/users/'.$module_info['users_id'].'/media/recent?access_token='.$module_info['access_user_token'];
				$json_output = json_decode(@file_get_contents($url), true);
				if ($json_output) {
