...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		// based on payment method selected we need to update mail content
      // Bank Transfer payment_method
     
       if ($order_info['payment_code'] == 'bank_transfer') {
        $mail->setHtml($this->load->view('mail/order_add_bt', $data));
      } elseif ($order_info['payment_code'] == 'ndis_checkout') {
        $mail->setSubject('NDIS payment under review');
        if (isset($this->session->data['ndis_plan_type']) && $this->session->data['ndis_plan_type'] !='') {
            switch ($this->session->data['ndis_plan_type']) {
                case 'Self Managed':
                    $mail->setHtml($this->load->view('mail/order_add_ndis_self_managed', $data));
                    break;
                case 'Plan Managed':
                    $mail->setHtml($this->load->view('mail/order_add_ndis_plan_managed', $data));
                    break;
                case 'NDIA Managed':
                    $mail->setHtml($this->load->view('mail/order_add_ndis_ndia_managed', $data));
                    break;
                default:
     ...
     error_log("PDF code started executing... ");
...
$data['payment_address'] = $order_info['payment_address_1'] . ', ' .
                           $order_info['payment_city'] . ', ' .
                           $order_info['payment_zone'] . ', ' .
                           $order_info['payment_country'];

$data['shipping_address'] = $order_info['shipping_address_1'] . ', ' .
