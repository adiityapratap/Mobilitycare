...
!(function () {
  ...
  var findPopovers = function (target) {
    var i;
    var popovers = document.querySelectorAll('.tooltip-popovers');
  ...
  window.addEventListener('click', function (event) {
    ...
    if (sideMenu) {
      ...
      if (hasBlocker === false) {
        ...
        blocker.setAttribute("onclick", "event.preventDefault();");
...
// FIX: Prevent page reload when clicking the blocker overlay (outside the menu)
document.body.addEventListener('click', function(e) {
    if (e.target && e.target.classList && e.target.classList.contains('side-menu-blocker')) {
        e.preventDefault();        // This stops the # jump / reload
        e.stopPropagation();      // Optional: prevents double-triggering
        
        // Properly close the menu (exactly like the theme expects)
        const activeMenu = document.querySelector('.side-menu.activeSideMenu, .side-menu.open');
        if (activeMenu) {
            activeMenu.classList.remove('activeSideMenu', 'open');
            activeMenu.classList.add('inactiveSideMenu');
        }
        e.target.remove();         // Remove the blocker overlay
    }
});