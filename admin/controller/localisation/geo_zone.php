...
class ControllerLocalisationGeoZone extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->post['postcode_ranges'])) {
   
           foreach ($this->request->post['postcode_ranges'] as $range) {
        $from = (int)$range['from'];
        $to   = (int)$range['to'];

        if ($from && $to && $from <= $to) {
            for ($pc = $from; $pc <= $to; $pc++) {
                $this->model_localisation_geo_zone->addGeoZonePostcode($geo_zone_id, $pc);
            }
        }
    }
           }
    ...
    foreach ($this->request->post['postcode_ranges'] as $range) {
        ...
        if ($from && $to && $from <= $to) {
            for ($pc = $from; $pc <= $to; $pc++) {
                $this->model_localisation_geo_zone->addGeoZonePostcode($this->request->get['geo_zone_id'], $pc);
            }
        }
    }
}
	...
	protected function getForm() {
		...
		if (isset($this->request->post['postcode_ranges'])) {
    // Coming back from form submission
    $data['postcode_ranges'] = $this->request->post['postcode_ranges'];
} elseif (!empty($geo_zone_info)) {
    // Fetch saved postcodes from DB
    $postcodes = $this->model_localisation_geo_zone->getGeoZonePostcodes($geo_zone_info['geo_zone_id']);

    // Re-group into ranges (optional: simple grouping for consecutive numbers)
    $ranges = [];
    $from = null;
    $prev = null;

    foreach ($postcodes as $postcode) {
        $pc = (int)$postcode['postcode'];

        if ($from === null) {
            $from = $pc;
            $prev = $pc;
        } elseif ($pc == $prev + 1) {
            $prev = $pc; // continue range
        } else {
            $ranges[] = ['from' => $from, 'to' => $prev];
            $from = $pc;
            $prev = $pc;
        }
    }
    ...
    $data['postcode_ranges'] = $ranges;
} else {
    $data['postcode_ranges'] = [];
}
