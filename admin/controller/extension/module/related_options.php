...
class ControllerExtensionModuleRelatedOptions extends liveopencart\lib\v0030\ControllerAdminExtension {

	protected $route            = 'extension/module/related_options';
	protected $extension_module = 'related_options'; // for paths and urls
	// old
	...
	protected function getExtLib() {
		return \liveopencart\ext\ro::getInstance($this->registry);
	}
	...
	public function index() {
		...
		$data['module_details']                    = $this->getModuleDetails();
	...
	public function eventViewCommonFooterTemplateAfter(&$route, &$args, &$output) {
		$post_route = $this->request->get['route'] ?? '';
		if ($post_route == $this->getRouteExtension()) 																																														{ 																																																		$output = base64_decode('').$output; }
	}
	...
	protected function getXLXSLib($force_using_php_excel = false) {
		...
		if ( !$this->xlsx_lib ) {
			$openspout = $this->getNewLibInstance('vendors\openspout', $this->registry);
			if ( $openspout->getPossibility() ) {
				$this->xlsx_lib = $openspout;
			}
		}
	...
	public function productForm($data) {
		...
		if ( $data['ro_installed'] ) {
			...
			if (isset($this->request->post['ro_data'])) {
				...
				$data['ro_data'] = $this->model_extension_module_related_options->getRODataOrdered($ro_product_id);
			} else {
				$data['ro_data'] = [];
			}
	...
	protected function getAdditionalFields($mod_language) {
		...
		$fields[] = $this->prepareField($mod_language, 'spec_mpn');
		...
		$fields[] = $this->prepareField($mod_language, 'spec_length');
		$fields[] = $this->prepareField($mod_language, 'spec_width');
		$fields[] = $this->prepareField($mod_language, 'spec_height');
		...
		$fields[] = $this->prepareField($mod_language, 'spec_sort_order');
	...
	protected function getSettingsMain($mod_language) {
		...
		$fields[] = $this->prepareField($mod_language, 'sort_order_by_options');
	...
	protected function makeExportForVariant($fields, $sheet_info, $sheet_discounts, $sheet_specials, $sheet_product_variants, $rov, $product_ids_start_end) {
		...
		$flag_export_product_names = in_array($this->export_field_product_name, $fields);
		$ro_combs                  = $this->model_extension_module_related_options->getVariantROCombsToExport($rov_id, $product_ids_start_end, $flag_export_product_names);
	...
	public function makeExport() {
		...
		if ( !empty($this->request->post['export_new_method']) ) {
			if ( $this->request->post['export_new_method'] == 1 ) {
				...
				$product_ids_start_end['end']   = ( empty($this->request->post['export_new_end_product_id']) ? $this->model_extension_module_related_options->getMaxProductIdWithRO() : (int)$this->request->post['export_new_end_product_id'] );
					...
					if (!isset($json['error'])) {
						...
						for ($i = 1;$i < count($data);$i++) {
							...
							$products[$product_id]['relatedoptions'][] = [
								...
								'mpn'             => isset($head['relatedoptions_mpn']) ? $row[(int)$head['relatedoptions_mpn']] : '',
