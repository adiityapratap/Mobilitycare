...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_catalog_product->saveProductSpecs($productId, $this->request->post);
		  ...
		  $product_features = array();
            if (isset($this->request->post['product_features'])) {
                $this->load->model('tool/image');
                $upload_dir = DIR_IMAGE . 'catalog/productFeature/';
                ...
                foreach ($this->request->post['product_features'] as $index => $feature) {
                    ...
                    if (isset($_FILES['product_features']['name'][$index]['image_file']) &&
                        $_FILES['product_features']['error'][$index]['image_file'] == UPLOAD_ERR_OK) {
                        $file = [
                            'name' => $_FILES['product_features']['name'][$index]['image_file'],
                            'type' => $_FILES['product_features']['type'][$index]['image_file'],
                            'tmp_name' => $_FILES['product_features']['tmp_name'][$index]['image_file'],
                            'error' => $_FILES['product_features']['error'][$index]['image_file'],
            ...
            if (empty($this->error)) {
                ...
                if ($this->request->server['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest') {
                    ...
                    $this->response->setOutput(json_encode(array(
                        'success' => true,
                        'redirect' => $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'], true)
                    )));
                    return;
                }
            } else {
...
private function uploadImage($file) {
        if (!empty($file['name'])) {
            ...
            $newFilename = 'spec_' . time() . '.' . $extension;
            $uploadPath = DIR_IMAGE . 'catalog/';
...
// Insert new messages
if (isset($this->request->post['dynamic_messages'])) {
    	// Delete old messages first
$this->db->query("DELETE FROM " . DB_PREFIX . "product_messages WHERE product_id = '" . (int)$product_id . "'");
    foreach ($this->request->post['dynamic_messages'] as $message) {
        if ($message) {
            $this->db->query("INSERT INTO " . DB_PREFIX . "product_messages SET product_id = '" . (int)$product_id . "', message = '" . $this->db->escape($message) . "'");
        }
    }
}
...
if (isset($this->request->post['product_specs'])) {
    ...
    $upload_dir = DIR_IMAGE . 'catalog/productSpecification/';
...
foreach ($this->request->post['product_specs'] as $index => $spec) {
    ...
    // Handle file upload
    if (isset($_FILES['product_specs']['name'][$index]['image_file']) &&
        $_FILES['product_specs']['error'][$index]['image_file'] == UPLOAD_ERR_OK) {
        $file = [
            'name' => $_FILES['product_specs']['name'][$index]['image_file'],
            'type' => $_FILES['product_specs']['type'][$index]['image_file'],
            'tmp_name' => $_FILES['product_specs']['tmp_name'][$index]['image_file'],
            'error' => $_FILES['product_specs']['error'][$index]['image_file'],
...
if (empty($this->error)) {
    ...
    $data['product_specs'] = $this->model_catalog_product->getProductSpecs($this->request->get['product_id']);
} else {
    $data['product_specs'] = [];
}
...
$product_messages = $this->db->query("SELECT message FROM " . DB_PREFIX . "product_messages WHERE product_id = '" . (int)$this->request->get['product_id'] . "'");
$data['dynamic_messages'] = array_column($product_messages->rows, 'message');

     // for region wise shipping cost 
       $this->load->model('localisation/geo_zone');
       $data['geo_zones'] = $this->model_localisation_geo_zone->getGeoZonesAndZones();
       
       $this->load->model('catalog/product');

if (isset($this->request->post['product_geo_zone_shipping'])) {
    $data['product_geo_zone_shipping'] = $this->request->post['product_geo_zone_shipping'];
} elseif (isset($this->request->get['product_id'])) {
    $data['product_geo_zone_shipping'] = $this->model_catalog_product->getProductGeoZoneShipping($this->request->get['product_id']);
} else {
    $data['product_geo_zone_shipping'] = [];
}
...
// this is for main homepage features products card image, homepage featured section 
$this->load->model('tool/image');
if (!empty($product_info) && is_file(DIR_IMAGE . $product_info['featured_image'])) {
    $data['featured_image_thumb'] = $this->model_tool_image->resize($product_info['featured_image'], 300, 300);
} else {
    $data['featured_image_thumb'] = $this->model_tool_image->resize('no_image.png', 300, 300);
}

$data['placeholderFeaturedImage'] = $this->model_tool_image->resize('no_image.png', 300, 300);
// end
		...
		// for display option to choose wtherer to displat cart button or not, addded by ady on 26th Jan 2025
		
		// In the addProduct and editProduct methods:

             if (isset($this->request->post['display_add_to_cart'])) {
               $data['display_add_to_cart'] = $this->request->post['display_add_to_cart'];
                } elseif (!empty($product_info)) {
               $data['display_add_to_cart'] = $product_info['display_add_to_cart'];
                } else {
                 $data['display_add_to_cart'] = 1; // Default value
                   }

          if (isset($this->request->post['display_custom_quote'])) {
           $data['display_custom_quote'] = $this->request->post['display_custom_quote'];
          } elseif (!empty($product_info)) {
        ...
        if (isset($this->request->get['product_id']) && $this->request->server['REQUEST_METHOD'] != 'POST') {
         ...
         } elseif (isset($this->request->post['product_features']) || isset($this->request->post['product_specs'])) {
         $data['product_features'] = $this->request->post['product_features'];
        ...
        // echo "<pre>"; print_r($product_features['product_specs']); exit;
