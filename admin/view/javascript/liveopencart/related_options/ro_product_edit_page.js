		...
		var ro_extension = {
			...
			getROTableHeadHTML: function(tab_num) {
				...
				if ( ro_settings.copy_comb_button ) {
					html += '<td width="40">';
					html += '<button type="button" onclick="ro_extension.copyCombinations('+tab_num+');" class="btn btn-default btn-xs"><i class="fa fa-plus-circle"></i><i class="fa fa-plus-circle"></i></button>';
					html += '</td>';
				}
				...
				if (ro_settings.spec_length || ro_settings.spec_width || ro_settings.spec_height) {
					html+= '<td class="text-left" width="90">';
					let html_parts = [];
					if (ro_settings.spec_length) {
						html_parts.push(ro_texts.entry_length);
					}
					if (ro_settings.spec_width) {
						html_parts.push(ro_texts.entry_width);
					}
					if (ro_settings.spec_height) {
						html_parts.push(ro_texts.entry_height);
					}
					html+= html_parts.join('<br>');
					html+= '</td>';
				}
				...
				if (ro_settings.spec_sort_order) {
					html+= '				<td class="text-left" width="90">'+ro_texts.entry_spec_sort_order+':</td>';
				}
			...
			getRONumByTr : function($tr) {
				return $tr.attr('id').replace('related-option', '');
			},
			...
			getROCombValuesFromForm : function(ro_num) {
				...
				ro_extension.triggerEvent('getROCombValuesFromForm_after', [ro_num, values]);

				return values;
			},

			copyCombinations : function(tab_num) {

				if (!confirm('Are you sure you want to copy all combinations?')) {
					return;
				}

				let $trs = $('#tbody-ro-'+tab_num+' tr[data-ro-tr-id]');
				$trs.each(function(){
					let ro_num = ro_extension.getRONumByTr($(this));
					ro_extension.copyCombination(ro_num);
				});
			},
			...
			addCombination : function(tab_num, params) {
				...
				str_add+= ro_extension.addTextField(tab_num, ro_num, 'spec_mpn', params, 'mpn');
				...
				if (ro_settings.spec_weight)	{
					...
					str_add+= '<option value="-" '+( (params.weight_prefix && params.weight_prefix=='-')? ('selected') : (''))+'>-</option>';
					str_add+= '</select>';
					str_add+= '<input type="text" class="form-control" name="'+input_name_prefix+'[weight]" value="'+(parseFloat(params.weight)||'')+'" size="5">';
					str_add+= '</td>';
				}
				...
				if (ro_settings.spec_length || ro_settings.spec_width || ro_settings.spec_height)	{
					str_add+= '<td>';
					if (ro_settings.spec_length) {
						str_add+= '<input type="text" class="form-control" name="'+input_name_prefix+'[length]" value="'+(parseFloat(params.length)||'')+'" size="5" title="'+ro_texts.entry_length+'">';
					}
					if (ro_settings.spec_width) {
						str_add+= '<input type="text" class="form-control" name="'+input_name_prefix+'[width]" value="'+(parseFloat(params.width)||'')+'" size="5" title="'+ro_texts.entry_width+'">';
					}
					if (ro_settings.spec_height) {
						str_add+= '<input type="text" class="form-control" name="'+input_name_prefix+'[height]" value="'+(parseFloat(params.height)||'')+'" size="5" title="'+ro_texts.entry_height+'">';
					}
					str_add+= '</td>';
				}
				...
				if (ro_settings.spec_sort_order) {
					str_add+= '<td><label class="control-label">&nbsp;</label>';
					let sort_order = ((params && typeof(params.sort_order) != 'undefined') ? params.sort_order : (ro_extension.getMaxSortOrderOnTab(tab_num)+1) );
					str_add+= '<input type="text" class="form-control" name="'+input_name_prefix+'[sort_order]" value="'+(sort_order)+'" >';
					str_add+= '</td>';	
				}
			...
			getMaxSortOrderOnTab: function(tab_num) {
				let sort_order_max = null;
				ro_extension.getTabElement(tab_num).find('input[name$="][sort_order]"]').each(function(){
					let val = parseInt($(this).val());
					sort_order_max = (sort_order_max === null ? val : Math.max(sort_order_max, val));
				});
				return sort_order_max;
			},
