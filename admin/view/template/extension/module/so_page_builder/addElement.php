		...
		if(file_exists(dirname(__FILE__).DIRECTORY_SEPARATOR.'shortcodes'.DIRECTORY_SEPARATOR.$shortcode.'_item'.DIRECTORY_SEPARATOR.'config.php')){
			...
			if(is_array($content)){
				...
				foreach($content[0]['cchild'] as $child => $value){
					...
					$html .= '<div data-icontitle="'.$database['language']->get('text_java_sortItem').'" data-placement="top"  class="element-sort element-icon"><i class="fa fa-arrows"></i></div>';
					$html .= '<div data-icontitle="'.$database['language']->get('text_java_deleteItem').'" data-placement="top"  class="element-delete element-icon"><i class="fa fa-remove"></i></div>';
