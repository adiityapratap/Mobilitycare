...
class Soconfig{
    ...
    public function css_out($CSSExcludes = '',$position = 'header'){
		...
		if ($this->get_settings('body_status'))$webfonts['body'] = $this->get_settings('body_font');
		
		if ($this->get_settings('h1_font_status'))$webfonts['h1'] = $this->get_settings('h1_font');
		if ($this->get_settings('h2_font_status'))$webfonts['h2'] = $this->get_settings('h2_font');
		if ($this->get_settings('h3_font_status'))$webfonts['h3'] = $this->get_settings('h3_font');
		if ($this->get_settings('h4_font_status'))$webfonts['h4'] = $this->get_settings('h4_font');
		
		if ($this->get_settings('navigation_font_status'))	$webfonts['.container-megamenu ul.megamenu > li > a'] = $this->get_settings('navigation_font');
		
		if ($this->get_settings('moduleTitle_font_status'))	$webfonts['#wrapper h3.modtitle,#wrapper h3.modtitle *'] = $this->get_settings('moduleTitle_font');
		if ($this->get_settings('productTitle_font_status'))	$webfonts['#wrapper  .product-item-container h4,#wrapper .title-product'] = $this->get_settings('productTitle_font');
		if ($this->get_settings('productPrice_font_status'))	$webfonts['#wrapper  .price'] = $this->get_settings('productPrice_font');
		
		if ($this->get_settings('custom_font_status')) $webfonts[$this->get_settings('customFont_content')] = $this->get_settings('custom_font');
		
		$this->addGoogleFont($webfonts);
		
		//Add Design Color Header
		$bgHeader = array();
		$bgHeader['#header'] = $this->get_settings('headerparent');
		$bgHeader['.header-top'] = $this->get_settings('headertop');
		$bgHeader['.header-middle'] = $this->get_settings('headercenter');
		$bgHeader['.header-bottom'] = $this->get_settings('headerbottom');
		
		if ($this->get_settings('headercolor_status')) $this->backgroundHeader($bgHeader);
...
if (isset($this->request->server['HTTPS']) && (($this->request->server['HTTPS'] == 'on') || ($this->request->server['HTTPS'] == '1'))) {
    $baseUrl = $this->oc_config->get('config_ssl');
} else {
    $baseUrl = $this->oc_config->get('config_url');
}
	...
	public function scss_compass($colorHex = null,$colorNameHex = null,$typelayout = null,$onoff_muticolor = null,$listColor = null){
		...
		if( !$this->is_admin() && $this->get_settings('scsscompile')) {
			...
			if(file_exists($scssDir.'header/header'.$typeheader.'.scss') ){
				...
				echo '<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> Please Create correct file path header'.$typelayout.'.scss </div>';
			}
			...
			if(file_exists($scssDir.'footer/footer'.$typefooter.'.scss') ){
				...
				echo '<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> Please Create correct file path footer'.$typelayout.'.scss </div>';
			}
	...
	public function scss_compassMobile($colorHex = null,$colorNameHex = null,$onoff_muticolor = null,$listColor = null){
		...
		$imagePath	= file_exists(DIR_TEMPLATE. $this->themes.'/images/styling/'.$colorName) ? $colorName: 'default';
		$themeColors  = ($colorHex != null)? $colorHex : $theme_color ;
		$themeNameColors = ($this->is_admin())? $colorNameHex : $this->get_settings('nameColor')  ;
	...
	public function getNameColor($typelayout=null){
	...
	public function backgroundHeader($bgHeader){
		
		echo '<style type="text/css">';
		foreach ($bgHeader as $className => $background){
			$background = json_decode($background);
			if(isset($background)){
				$StyleCSS = $className . "{";
				if (isset($background->background) && $background->background) $StyleCSS .= 'background: ' . $background->background . ' !important;';
				if (isset($background->color) && $background->color) $StyleCSS .= 'color: ' . $background->color . '!important;';
				$StyleCSS .= "}";
				echo $StyleCSS;
			}
		}
        ...
        if (is_array($fonts) && !empty($fonts)){
			...
			foreach ($fonts as $key => $font){
				...
				if(isset($font)){
					...
					if (isset($font->fontSize) && $font->fontSize)$fontCSS .= 'font-size: ' . $font->fontSize . 'px !important;';
					if (isset($font->fontWeight) && $font->fontWeight)$fontCSS .= 'font-weight: ' . $font->fontWeight . ' !important;';
					if (isset($font->fontStyle) && $font->fontStyle)  $fontCSS .= 'text-transform: ' . $font->fontStyle . '!important;';
