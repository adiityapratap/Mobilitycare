...
class ModelExtensionModuleSomegamenu extends Model {
    ...
    public function addMenu($data) {
		...
		$data['label_item'] = !empty($data['label_item'])?$data['label_item']:'0';
		$data['icon_font'] = !empty($data['icon_font'])?$data['icon_font']:'0';
		$data['class_menu'] = !empty($data['class_menu'])?$data['class_menu']:'0';
			...
			foreach ($parent_menu as $menu) {
				...
				foreach ($subcategories as $result) {
					$data = array(
							...
							'content_type' => $result['content_type'],
							'content' => json_decode($result['content'], true),
							'list_categories' => (isset($result['content']['categories']['categories']) && $result['content']['categories']['categories']) ? $this->model_extension_module_so_megamenu->getCategories(json_decode($result['content']['categories']['categories'], true)) : ''
						);
