...
class ModelExtensionModuleRelatedOptions extends \liveopencart\lib\v0030\Model {
	
	use \liveopencart\lib\v0030\traits\cache;
	...
	protected function getROVariant($rov_id) {
		return $this->getSimpleDB()->selectRowById('relatedoptions_variant', $rov_id);
	}
	...
	public function getVariantROCombsToExport($rov_id, $product_ids_start_end = [], $flag_export_product_names = false) {
		
		// get data to export (ro combs)
		$query = $this->db->query("
			SELECT RO.*, P.model product_model
				".
				($flag_export_product_names ? " , PD.name product_name_info_only " : "")
				."
			FROM ".DB_PREFIX."relatedoptions RO
				,".DB_PREFIX."relatedoptions_variant_product ROVP
				,".DB_PREFIX."product P
					".
					($flag_export_product_names ? " LEFT JOIN ".DB_PREFIX."product_description PD ON (PD.product_id = P.product_id) " : "")
					."
			WHERE ROVP.relatedoptions_variant_id = ".(int)$rov_id."
			  AND ROVP.relatedoptions_variant_product_id = RO.relatedoptions_variant_product_id
			  AND RO.product_id = P.product_id
				".
				($flag_export_product_names ? " AND (PD.language_id IS NULL OR PD.language_id = ".(int)$this->config->get('config_language_id').") " : "")
				."
				".( $product_ids_start_end ? " AND RO.product_id BETWEEN ".(int)$product_ids_start_end['start']." AND ".(int)$product_ids_start_end['end']." " : "" )."
	...
	public function setVariantsOfRelatedOptions($ro_variants, $clear_others = true) {
		...
		$updated_ro_variant_ids = [];
		
		if (is_array($ro_variants)) {
			...
			foreach ($ro_variants as $ro_variant) {
				
				if (is_array($ro_variant)) {
					
					$ro_variant_id   = (int)(isset($ro_variant['id'])) ? $ro_variant['id'] : "";
					$vo_name = "";
					if (isset($ro_variant['name']) && $ro_variant['name'] != '' ) {
						$vo_name = $ro_variant['name'];
					} else {
						if (isset($ro_variant['options']) && is_array($ro_variant['options'])) {
							$lang_id    = $this->config->get('config_language_id');
							$options_in = implode(",", array_map('intval', array_values($ro_variant['options'])) );
							if ( $options_in ) {
								$query = $this->db->query("
									SELECT *
					...
					if (!empty($ro_variant_id)) {
						$query = $this->db->query("
							UPDATE `".DB_PREFIX."relatedoptions_variant`
							SET relatedoptions_variant_name='".$this->db->escape($vo_name)."'
							  , sort_order= ".(int)(isset($ro_variant['sort_order']) ? $ro_variant['sort_order'] : 0 )."
							WHERE relatedoptions_variant_id = ".$ro_variant_id."
						");
					} else {
						$query = $this->db->query("
							...
							SET relatedoptions_variant_name='".$this->db->escape($vo_name)."'
							  , sort_order= ".(int)(isset($ro_variant['sort_order']) ? $ro_variant['sort_order'] : 0 )."
						");
						$ro_variant_id = $this->db->getLastId();
					}
					$str_ro_variant_ids .= ",".$ro_variant_id;
					$updated_ro_variant_ids[] = $ro_variant_id;
					
					if (isset($ro_variant['options'])) {
						$vo_options_ids = $ro_variant['options'];
						if (is_array($vo_options_ids)) {
							$vo_options_ids = array_unique(array_values($vo_options_ids));
							
							$vo_options_ids_old = (isset($all_vo_options_ids_old[$ro_variant_id]) ? $all_vo_options_ids_old[$ro_variant_id] : []);
							//$query = $this->db->query("	SELECT option_id FROM `".DB_PREFIX."relatedoptions_variant_option` WHERE relatedoptions_variant_id=".(int)$ro_variant_id."");
							...
							if ($vo_options_ids_to_remove) {
								$query = $this->db->query("
									...
									WHERE relatedoptions_variant_id=".(int)$ro_variant_id."
									  AND option_id IN (".implode(',', array_map('intval', $vo_options_ids_to_remove)).")
								");
							}

							$query_doubles = $this->db->query("
								SELECT COUNT(*) AS cnt, option_id
								FROM `".DB_PREFIX."relatedoptions_variant_option` 
								WHERE relatedoptions_variant_id=".(int)$ro_variant_id."
								GROUP BY option_id
							");

							foreach ($query_doubles->rows as $row_double) {
							 	if ($row_double['cnt'] > 1) {

									$query = $this->db->query("
										DELETE FROM `".DB_PREFIX."relatedoptions_variant_option`
										WHERE relatedoptions_variant_id = ".(int)$ro_variant_id."
											AND option_id = ".(int)$row_double['option_id']."
										LIMIT ".(int)($row_double['cnt']-1)."
									");
								}
							}
							...
							if ($vo_options_ids_to_add) {
								foreach ($vo_options_ids_to_add as $opts_key => $option_id) {
									...
									// $query = $this->db->query("	DELETE FROM `".DB_PREFIX."relatedoptions_variant_option` WHERE relatedoptions_variant_id=".(int)$ro_variant_id." AND option_id = ".(int)$option_id." ");
									
									$query = $this->db->query("	INSERT INTO `".DB_PREFIX."relatedoptions_variant_option` SET relatedoptions_variant_id=".(int)$ro_variant_id.", option_id = ".(int)$option_id." ");
								}
							}
						}
					}
					
					$this->liveopencart_ext_ro->callPRO('updateVariantFields', [$ro_variant_id, !empty($ro_variant['fields']) ? $ro_variant['fields'] : []]);
				}
		...
		if ($clear_others) {
			...
			if ($clear_others) {
				$query = $this->db->query("	DELETE FROM `".DB_PREFIX."relatedoptions_variant_option` WHERE NOT relatedoptions_variant_id IN (0".$str_ro_variant_ids.") ");
			}
			
			$this->removeVariants(" NOT relatedoptions_variant_id IN (0".$str_ro_variant_ids.") ");
			//$query = $this->db->query("	DELETE FROM `".DB_PREFIX."relatedoptions_variant` WHERE NOT relatedoptions_variant_id IN (0".$str_ro_variant_ids.") ");
			//$query = $this->db->query("	DELETE FROM `".DB_PREFIX."relatedoptions_variant_product` WHERE NOT relatedoptions_variant_id IN (0".$str_ro_variant_ids.") ");
		}
		
		$this->liveopencart_ext_ro->callPRO('removeFieldsOfInexistentVariants');
		
		return $updated_ro_variant_ids; //rov_ids
	...
	protected function getProductROOptionsGrouppedByROVPId($product_id) {
		$rovps_ros_options = [];
		$query             = $this->db->query("
				SELECT ROO.*, RO.relatedoptions_variant_product_id, OV.sort_order
				FROM `" . DB_PREFIX . "relatedoptions_option` ROO
					,`" . DB_PREFIX . "relatedoptions` RO
					,`" . DB_PREFIX . "option` O
	...
	protected function getOptionsIdsGrouppedByROVId() {
		...
		$query                = $this->db->query("
			...
			FROM `" . DB_PREFIX . "relatedoptions_variant_option` ROVO
				,`" . DB_PREFIX . "option` O
			WHERE ROVO.option_id = O.option_id
			ORDER BY O.sort_order ASC, ROVO.option_id ASC
		");
	...
	public function getROData($product_id, $with_char_id = false) {
		...
		foreach ($rovp_rows as $rovp_row) {
			...
			$ro_data[] = [
				...
				'use'               => (int)$rovp_row['relatedoptions_use'],
				'rov_id'            => $rovp_row['relatedoptions_variant_id'],
				'allow_zero_select' => (int)$rovp_row['allow_zero_select'],
				'ro'                => [],
				'options_ids'       => [],
			...
			if (isset($rovps_ros_ros_options[$rovp_id])) {
				...
				//foreach ($query->rows as $row) {
					...
					$ro_data[$cnt]['ro'][$row['relatedoptions_id']]['ov_sort_order'.$row['option_id']] = $row['sort_order'];
				}
			}
	...
	public function getRODataOrdered($product_id) {
		// make sort order here for comp with all legacy code (where ro in ro_data are assoc by ro_id)
		$ro_data = $this->getROData($product_id);
		
		return $this->liveopencart_ext_ro->orderRODataByOVSortOrder($ro_data);
		
		//$ro_settings = $this->config->get('related_options');
		//
		//if (!empty($ro_settings['spec_sort_order'])) {
		//	foreach ($ro_data as &$ro_dt) {
		//		$ro_dt['ro'] = array_values($ro_dt['ro']);
		//		array_multisort(array_column($ro_dt['ro'], 'sort_order'), SORT_ASC, $ro_dt['ro']);
		//	}
		//	unset($ro_dt);
		//} elseif (!empty($ro_settings['sort_order_by_options'])) {
		//	
		//	foreach ($ro_data as &$ro_dt) {
		//		$ro_dt['ro']    = array_values($ro_dt['ro']);
		//		$sort_arguments = [];
		//		foreach ($ro_dt['options_ids'] as $option_id) {
		//			$sort_arguments[] = array_column($ro_dt['ro'], 'ov_sort_order'.$option_id);
		//		}
		//		$sort_arguments[] = SORT_ASC;
		//		$sort_arguments[] = &$ro_dt['ro'];
		//		
		//		call_user_func_array('array_multisort', $sort_arguments);
		//		
		//		//array_multisort(array_column($ro_dt['ro'], 'sort_order'), SORT_ASC, $ro_dt['ro']);
		//	}
		//	unset($ro_dt);
	...
	public function setROData($product_id, $data) {
		...
		if ( (isset($data['ro_data_included']) && $data['ro_data_included']) || (isset($data['ro_data']) && $data['ro_data']) ) {
			...
			$this->updateProductDateModified($product_id);
		}
	...
	public function hasProductRO($product_id) {
		$query = $this->db->query("SELECT COUNT(relatedoptions_id) cnt FROM `".DB_PREFIX."relatedoptions` WHERE product_id = ".(int)$product_id." ");
	...
	public function updateStandardProductDataByRO($product_id, $options = [], $product_options_used_in_ro_before = [], $ro_quantity = 0) {
		...
		// update product quantity (but only if the product really has ro combs)
		if ( $mod_settings && isset($mod_settings['update_quantity']) && $mod_settings['update_quantity'] && $this->hasProductRO($product_id) ) {
			...
			if ( isset($data['ro']) && (is_array($data['ro']))  ) {
				...
				if (count($options) != 0) {
					...
					foreach ($data['ro'] as $relatedoption) {
						...
						$defaults_empty = ['model', 'sku', 'upc', 'ean', 'jan', 'mpn', 'location', 'stock_status_id', 'in_stock_status_id', 'weight_prefix', 'weight', 'length', 'width', 'height', 'price', 'defaultselect', 'defaultselectpriority', 'disabled', 'sort_order'];
						...
						$set_sql = "
							SET	product_id = " . (int)$product_id . "
							  ...
							  , mpn = '".$this->db->escape((string)$relatedoption['mpn'])."'
							  ...
							  , length = ".(float)$relatedoption['length']."
							  , width = ".(float)$relatedoption['width']."
							  , height = ".(float)$relatedoption['height']."
							  ...
							  , sort_order = ".(int)$relatedoption['sort_order']."
						";
	...
	protected function removeROCombsByFilter($sql_where = "") {
		if ( !$sql_where ) { // remove all combinations of related options
			...
			$query_ro_products = $this->db->query("SELECT DISTINCT product_id FROM ".DB_PREFIX."relatedoptions WHERE ".$sql_where);
			...
			// "DELETE " . DB_PREFIX . "relatedoptions_option.* FROM " allows to use indexes
			$this->db->query("DELETE " . DB_PREFIX . "relatedoptions_option.* FROM " . DB_PREFIX . "relatedoptions_option WHERE relatedoptions_id IN ( ".$ro_comb_sql." )");
			...
			foreach ($query_ro_products->rows as $row) {
				$this->updateProductDateModified($row['product_id']);
			}
		}
	...
	public function getOrderInfoPageData($data) {
		...
		if ($data['ro_installed'] && $ro_settings)  {
			...
			$data['column_mpn']      = $this->language->get('entry_mpn');
	...
	protected function checkTables($install = false) {
		...
		if ( $query->num_rows ) {
			...
			if ( $this->installed() || $install ) {
				$this->getSimpleDB()->addTableColumnIfNotExists('relatedoptions_variant_product', 'allow_zero_select', 'TINYINT(1) NOT NULL');
				$this->getSimpleDB()->addTableColumnIfNotExists('relatedoptions', 'in_stock_status_id', 'INT(11) NOT NULL');
				$this->getSimpleDB()->addTableColumnIfNotExists('relatedoptions', 'disabled', 'TINYINT(1) NOT NULL');
				$this->getSimpleDB()->addTableColumnIfNotExists('relatedoptions', 'jan', 'VARCHAR(13) NOT NULL');
				$this->getSimpleDB()->addTableColumnIfNotExists('relatedoptions', 'mpn', 'VARCHAR(70) NOT NULL');
				$this->getSimpleDB()->addTableColumnIfNotExists('relatedoptions', 'sort_order', 'INT(11) NOT NULL');
				$this->getSimpleDB()->addTableIndexIfNotExists('relatedoptions', 'disabled');
				
				$this->getSimpleDB()->addTableColumnIfNotExists('relatedoptions', 'length', 'decimal(15,8) NOT NULL DEFAULT 0.00000000');
				$this->getSimpleDB()->addTableColumnIfNotExists('relatedoptions', 'width', 'decimal(15,8) NOT NULL DEFAULT 0.00000000');
				$this->getSimpleDB()->addTableColumnIfNotExists('relatedoptions', 'height', 'decimal(15,8) NOT NULL DEFAULT 0.00000000');
	...
	protected function updateProductDateModified($product_id) {
		if ($product_id) {
